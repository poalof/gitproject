...
class IndexScanExecutor : public AbstractExecutor {
 public:
  ...
  static bool cmp(const RowId& a, const RowId& b) //直接写bool类型cmp函数
  {
    return a.GetPageId() < b.GetPageId() 
        || a.GetPageId() == b.GetPageId() && a.GetSlotNum() < b.GetSlotNum();
  }

  void GetMap(const AbstractExpressionRef &filter_predicate_, std::map<uint32_t, pair<Field, std::string>>& col_map);

 private:

  /** The sequential scan plan node to be executed */
  const IndexScanPlanNode *plan_;
  std::vector<RowId> results; // *这也我加
  int count{0};
};