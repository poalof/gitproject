...
int InternalPage::InsertNodeAfter(const page_id_t &old_value, GenericKey *new_key, const page_id_t &new_value) {
  ...
  PairCopy(PairPtrAt(idx+1)+pair_size, PairPtrAt(idx+1), GetSize()-idx-1);
  SetKeyAt(idx+1, new_key);
 ...
 * buffer_pool_manager �?干嘛的？传给CopyNFrom()用于Fetch数据�?
 */
void InternalPage::MoveHalfTo(InternalPage *recipient, BufferPoolManager *buffer_pool_manager) {
  int start = GetSize() / 2 ;
  int size = GetSize() - start;
  recipient->CopyNFrom(PairPtrAt(start), size, buffer_pool_manager);
  IncreaseSize(-1 * size);
}
...
void InternalPage::CopyNFrom(void *src, int size, BufferPoolManager *buffer_pool_manager) {
  ...
  {
    auto page = buffer_pool_manager->FetchPage(ValueAt(GetSize()+i));
    if (page != nullptr) 
    {
...
void InternalPage::MoveAllTo(InternalPage *recipient, GenericKey *middle_key, BufferPoolManager *buffer_pool_manager) {
  SetKeyAt(0, middle_key); // i dont understand???
...
void InternalPage::CopyLastFrom(GenericKey *key, const page_id_t value, BufferPoolManager *buffer_pool_manager) {
  ...
  auto page = buffer_pool_manager->FetchPage(ValueAt(GetSize()));
  if(page != nullptr)
  {
    auto node = reinterpret_cast<BPlusTreePage*>(page->GetData());
    node->SetParentPageId(GetPageId());
